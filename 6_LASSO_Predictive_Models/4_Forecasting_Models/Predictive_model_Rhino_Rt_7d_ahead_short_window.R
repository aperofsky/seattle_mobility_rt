...
rhino_df <- combined %>%
  select(date, matches(cols2), rhino, covid) %>%
  dplyr::select(date, rhino, covid, contains("lag")) %>%
  filter(date >= as.Date("2019-01-22") & date <= as.Date("2022-05-16")) %>%
  complete(date = seq.Date(as.Date("2019-01-22"), as.Date("2022-05-16"), by = "day")) %>%
  filter(!is.na(fb_leaving_home_custom_lag1))

rhino_df <- rhino_df %>% replace_na(list(covid = 0))
data.table::data.table(rhino_df[!complete.cases(rhino_df), ])
########################################################################################
## Moving window LASSO predictive model
########################################################################################
## limit to after vaccination and Delta emergence
rhino_df_scaled <- rhino_df
rhino_df_scaled[!complete.cases(rhino_df_scaled), ]
head(rhino_df_scaled) # 2019-01-22
data.table::data.table(rhino_df_scaled[!complete.cases(rhino_df_scaled), ])
...
names(rhino_df_scaled)
var_target <- "rhino" # dependent variable
var_regressors_GT <- select(rhino_df_scaled, -date, -rhino, -covid, -c(DailyAverageRelativeHumidity_lag1:DailyAverageWetBulbTemperature_lag14)) %>% names()
var_regressors_GT
var_regressors_GT_COVID <- select(rhino_df_scaled, -date, -rhino, -c(DailyAverageRelativeHumidity_lag1:DailyAverageWetBulbTemperature_lag14)) %>% names()
var_regressors_GT_COVID
var_regressors_climate <- select(rhino_df_scaled, -date, -rhino, -covid) %>% names()
...
names(glm_rhino)
range(glm_rhino$date) # "2019-02-06" "2022-05-16"
...
for (t_predict_start in t0_predict_start:tmax_predict_start) {
  ...
  glm_predict <- filter(glm_rhino, num_days >= t_predict_start & num_days <= t_predict_end) # predict up to 7 days ahead
...
r <- ggplot() +
  ...
  geom_line(
    data = rhino_results %>% filter(date >= as.Date("2019-01-22") & date <= as.Date("2022-05-16")),
  ...
  geom_line(
    ...
    aes(x = new_date, y = data_7d_ahead, colour = "Rhinovirus Rt"), linewidth = 1.7
  ) +
  ...
  scale_color_manual(
    ...
    labels = c("Rhinovirus Rt", "AR", "AR + Mobility", "AR + Climate", "AR + Mobility + Climate", "AR + Mobility + Climate + SARS-CoV-2 Rt"),
    breaks = c("Rhinovirus Rt", "AR", "SG-AR", "Climate-AR", "Mobility-Climate-AR", "AR-SG-COVID-Climate")
  ) +
  background_grid() +
...
p <- ggplot() +
  ...
  scale_color_manual(
    name = "Model",
    # values = c("#ABDDA4", as.vector(jcolors('pal5'))),
    values = c("#ABDDA4", "#FDAE61", "#2B83BA", "navy", "purple", "#D7191C"),
    breaks = c("Rhinovirus Rt", "AR", "AR + Mobility", "AR + Climate", "AR + Mobility + Climate", "AR + Mobility + Climate + SARS-CoV-2 Rt")
  ) +
  xlab("Date") +
...
com
save_plot(com, filename = "figures/fig_s20_rhinovirus_rt_predictive_model_fit_7d_ahead_updated.png", base_width = 12, base_height = 10)
