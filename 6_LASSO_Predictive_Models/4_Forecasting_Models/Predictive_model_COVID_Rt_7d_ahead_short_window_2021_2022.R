...
covid_vax <- read.csv("6_LASSO_Predictive_Models/2_Vaccination_coverage_data/KC_vax_public_2024_01_24.csv") %>% dplyr::select(date, Perc_Full_Vax)
covid_vax$date <- as.Date(covid_vax$date, format = "%m/%d/%Y")

combined <- left_join(combined, covid_vax, by = "date") %>% replace_na(list(Perc_Full_Vax = 0))

covid_voc <- read_rds("6_LASSO_Predictive_Models/3_VOC_data/KC_WA_covid_cases_by_variant_2020_2022.rds")
covid_voc_wide <- covid_voc %>%
  rename(voc = clade_new) %>%
  dplyr::select(date, voc, pred_freq) %>%
  pivot_wider(names_from = voc, values_from = pred_freq) %>%
  dplyr::select(date, Ancestral, Alpha, Delta, `Omicron BA.1`, `Omicron BA.2`) %>%
  rename(omicron_ba1 = `Omicron BA.1`, omicron_ba2 = `Omicron BA.2`) %>%
  mutate(across(where(is.numeric), ~ round(.x, 3)))
head(covid_voc_wide)

combined <- left_join(combined, covid_voc_wide, by = "date")
...
covid_df_scaled <- covid_df %>% filter(Perc_Full_Vax_lag14 > 0)
covid_df_scaled[!complete.cases(covid_df_scaled), ]
head(covid_df_scaled) # 2021-01-14
...
var_regressors_GT <- select(covid_df_scaled, -date, -rhino, -covid, -c(Perc_Full_Vax_lag1:DailyAverageWetBulbTemperature_lag14)) %>% names()
var_regressors_GT
var_regressors_GT_RHINO <- select(covid_df_scaled, -date, -covid, -c(Perc_Full_Vax_lag1:DailyAverageWetBulbTemperature_lag14)) %>% names()
var_regressors_GT_RHINO
var_regressors_vax_variant <- select(covid_df_scaled, -date, -rhino, -covid, -c(DailyAverageRelativeHumidity_lag1:DailyAverageWetBulbTemperature_lag14)) %>% names()
var_regressors_vax_variant
var_regressors_climate <- select(covid_df_scaled, -date, -rhino, -covid, -c(Perc_Full_Vax_lag1:omicron_ba1_lag14)) %>% names()
var_regressors_climate
var_regressors_AR <- sprintf("AR%02d", lag_AR:(lag_AR + window_AR - 1)) # regressors, autoregressive
var_regressors_AR
var_regressors_ARGO <- unique(c(var_regressors_AR, var_regressors_GT_RHINO, var_regressors_vax_variant, var_regressors_climate)) # all regressors, ARGO
var_regressors_ARGO
...
sgtrends_covid <- covid_df_scaled %>%
  ...
  dplyr::select(
    date, all_of(unique(c(var_regressors_GT, var_regressors_GT_RHINO, var_regressors_vax_variant, var_regressors_climate))),
    num_days, epi_year, epi_week
  ) %>%
  pivot_longer(
    cols = all_of(unique(c(var_regressors_GT, var_regressors_GT_RHINO, var_regressors_vax_variant, var_regressors_climate))),
...
names(glm_covid)
glm_covid %>% filter(Perc_Full_Vax_lag14 > 0) # 2021-01-29
glm_covid %>% filter(Delta_lag1 > 0) # 2021-04-01
range(glm_covid$date) # "2021-01-29" "2022-05-16"

res_GT <- data.frame()
res_GT_AR <- data.frame()
res_GT_RHINO <- data.frame()
res_vax_variant <- data.frame()
...
for (t_predict_start in t0_predict_start:tmax_predict_start) {
  ...
  x_predict_vax_variant <- data.matrix(glm_predict[, var_regressors_vax_variant])
  ...
  cvfit_vax_variant <- cv.glmnet(x_train_vax_variant, y_train, weights = w, type.measure = "mse", nfolds = nfolds, alpha = alpha)
  ...
  coef_vax_variant <- cvfit_vax_variant %>%
    coef(s = s) %>%
    as.matrix() %>%
    t() %>%
    as.data.frame() %>%
  ...
  predict_vax_variant <- cvfit_vax_variant %>%
    predict(newx = x_predict_vax_variant, s = s) %>%
    t() %>%
    data.frame()
  ...
  colnames(predict_vax_variant) <- var_predicts
  ...
  res_vax_variant <- bind_rows(res_vax_variant, cbind(data.frame("num_days" = t_train_end, "date" = date_train_end), data_epi, predict_vax_variant, coef_vax_variant))
  head(res_vax_variant)
...
label_df <- data.frame(
  ...
  label = c(
    ...
    "Prop. Delta, Lag 14",
    # "Prop. Omicron BA.1",
    "Prop. Omicron BA.1, Lag 1",
    "Prop. Omicron BA.1, Lag 7",
    "Prop. Omicron BA.1, Lag 14",
    # "Prop. Omicron BA.2",
    # "Relative Humidity",
...
ci_df <- rt_df %>%
  ...
  filter(level == 90) %>%
  filter(date >= as.Date("2021-02-27") - 30)
range(ci_df$date)
...
covid_cases <- covid_cases %>%
  complete(date = seq.Date(as.Date("2020-01-01"), as.Date("2022-05-16"), by = "day")) %>%
...
r <- ggplot() +
  ...
  geom_rect(
    aes(
      xmin = as.Date("2021-02-27") - 30,
      xmax = as.Date("2021-03-06"),
      ymin = -Inf, ymax = Inf
    ),
    alpha = 0.1, fill = "yellow"
  ) +
  geom_ribbon(
    data = covid_cases %>% filter(date >= as.Date("2021-02-27") - 30 & date <= as.Date("2022-05-16")),
  ...
  geom_line(
    data = ci_df %>% filter(date >= as.Date("2021-02-27") - 30 & date <= as.Date("2022-05-16")),
    aes(x = date, y = median),
  ...
  geom_line(
    data = res_mob_climate_rhino_AR %>% mutate(new_date = date + 7),
    aes(x = new_date, y = predict_7d_ahead, colour = "Mobility-Climate-Rhino-AR"), linewidth = 1
  ) +
  geom_line(data = res_ARGO %>% mutate(new_date = date + 7), aes(x = new_date, y = predict_7d_ahead, colour = "AR-SG-RHINO-Climate-Vax-Variant"), linewidth = 1) +
  geom_line(data = res_AR %>% mutate(new_date = date + 7), aes(x = new_date, y = predict_7d_ahead, colour = "AR"), linewidth = 1) +
  geom_line(data = res_ARGO2 %>% mutate(new_date = date + 7), aes(x = new_date, y = data_7d_ahead, colour = "SARS-CoV-2 Rt"), linewidth = 1.7) +
  ...
  geom_vline(xintercept = as.Date("2021-03-06"), lty = "dashed", color = "yellow") +
  scale_color_manual(
    name = NULL,
    values = c("#ABDDA4", "#FDAE61", "#2B83BA", "navy", "purple", "#D7191C", "pink"),
    breaks = c("SARS-CoV-2 Rt", "AR", "SG-AR", "Climate-AR", "Mobility-Climate-AR", "Mobility-Climate-Rhino-AR", "AR-SG-RHINO-Climate-Vax-Variant"),
    labels = c("SARS-CoV-2 Rt", "AR", "AR + Mobility", "AR + Climate", "AR + Mobility + Climate", "AR + Mobility + Climate + Rhino Rt", "AR + Mobility + Climate + Rhino Rt + Vax + Variants")
  ) +
  background_grid() +
...
########################################################################################
## Predictive accuracy: Table S5
########################################################################################
model_predict_df <- bind_rows(
  res_ARGO %>% mutate(model = "AR + Mobility + Climate + Rhino Rt + Vax + Variants"),
  ...
  res_mob_climate_rhino_AR %>% mutate(model = "AR + Mobility + Climate + Rhino Rt")
)
...
range(all_residuals$error)
range(all_residuals$percent_error)
...
p <- ggplot() +
  ...
  scale_x_date(
    ...
    limits = c(as.Date("2021-02-27") - 30, as.Date("2022-05-16"))
  ) +
  ...
  scale_color_manual(
    name = "Model",
    values = c("#ABDDA4", "#FDAE61", "#2B83BA", "navy", "purple", "#D7191C", "pink"),
    breaks = c("SARS-CoV-2 Rt", "AR", "AR + Mobility", "AR + Climate", "AR + Mobility + Climate", "AR + Mobility + Climate + Rhino Rt", "AR + Mobility + Climate + Rhino Rt + Vax + Variants")
  ) +
  xlab("Date") +
...
com
# save_plot(com, filename = "figures/fig_s19_covid_rt_predictive_model_fit_7d_ahead_2021_2022.png", base_width = 12, base_height = 10)
