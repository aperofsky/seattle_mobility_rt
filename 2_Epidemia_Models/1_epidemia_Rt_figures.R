...
###################################################
## Fig. S27 Comparison between SCAN COVID incidence vs county cases
###################################################
...
pre_omicron_comparison <- ggplot() +
  ...
  geom_line(aes(x = date, y = total_cases / 65000, color = "cases"),
    data = covid_cases_all_voc %>% filter(date < as.Date("2021-12-01")), linetype = "solid"
  ) +
  scale_y_continuous(
...
post_omicron_comparison <- ggplot() +
  ...
  geom_line(aes(x = date, y = total_cases / 65000, color = "cases"), data = covid_cases_all_voc %>%
    filter(date >= as.Date("2021-12-01") & date < as.Date("2022-07-01")), linetype = "solid") +
  scale_y_continuous(
    # Features of the first axis
    name = "SCAN COVID-19 Incidence",
    # Add a second axis and specify its features
...
covid_comparison2
save_plot(covid_comparison2, filename = "figures/fig_s27_scan_vs_county_covid_comparison.png", base_width = 20, base_height = 8)
...
end_and_cov
save_plot(end_and_cov, filename = "figures/fig_1_rt_endemic_and_covid_combined.png", base_width = 16, base_height = 16)
...
all_endemic_results <- bind_rows(
  ...
  all_IVB_rt, all_entero_rt,
  all_rhino_rt, all_HPIV_3_4_rt, all_HPIV_1_2_rt,
...
all_endemic_results$organism <- as.factor(all_endemic_results$organism)
levels(all_endemic_results$organism)[c(2, 6:9)]
levels(all_endemic_results$organism)[c(2, 6:9)] <- c("EV", "IAV/H1N1", "IAV/H3N2", "IBV", "RV")
...
all_rt
unique(all_rt$organism)
write.csv(all_rt, file = "2_Epidemia_Models/rt_all_pathogens_raw.csv", row.names = F)
write.csv(all_rt, file = "4_Block_Bootstrap_Mobility_vs_Rt/rt_all_pathogens_raw.csv", row.names = F)

all_rt_mv_avg <- bind_rows(all_endemic_results_avg, all_covid_rt_avg)
head(all_rt_mv_avg)
unique(all_rt_mv_avg$organism)
write.csv(all_rt_mv_avg, file = "2_Epidemia_Models/rt_all_pathogens_15day_mv_avg.csv", row.names = F)
write.csv(all_rt_mv_avg, file = "4_Block_Bootstrap_Mobility_vs_Rt/rt_all_pathogens_15day_mv_avg.csv", row.names = F)