...
####################################
## Enterovirus 2018 - 2022
####################################
Entero_2018_2022 <- filter_fun(pathogen_df %>% filter(date > as.Date("2018-11-15") & date < as.Date("2022-08-30"))) %>%
  filter(date > as.Date("2018-11-29") & date < as.Date("2022-07-01"))
# range(Entero_2018_2022$date)
# ggplot(Entero_2018_2022)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2018-11-29") + seq(0, along.with = c(NA, Entero_2018_2022$confirm))

data <- data.frame(
  city = "Seattle", cases = c(NA, Entero_2018_2022$confirm),
  date = date
)
...
entero_si <- R0::generation.time("gamma", c(4.4, 2.7), truncate = 21)
## truncate at 21 days because hRV can be shed for a few weeks

# basic model
inf <- epiinf(gen = entero_si$GT)

# extended model
# treat infections as latent parameters
inf_ext <- epiinf(
  gen = entero_si$GT, latent = TRUE,
...
write_rds(fm2, file = "//data//perofskyamc//Entero_2018_2022_biowulf_epidemia_model.rds")

####################################
## extract Rt
####################################

entero_rt_2018_2022 <- extract_Rt(fm2)

all_entero_rt <- entero_rt_2018_2022 %>%
  group_by(group, tag, level) %>%
  tidyr::complete(date = full_seq(date, 1)) %>%
  ungroup()
all_entero_rt$organism <- "Enterovirus"

write_rds(all_entero_rt, file = "//data//perofskyamc//Entero_2018_2022_biowulf_epidemia_model_rt_only.rds") # rt only
