...
## HPIV 2020
####################################
HPIV_2020 <- filter_fun(pathogen_df %>% filter(date>as.Date("2019-08-01") & date<as.Date("2020-04-15"))) %>% filter(date<as.Date("2020-02-15") & date > as.Date("2019-08-23"))
# ggplot(HPIV_2020) +
#   geom_line(aes(x = date, y = confirm))
# range(HPIV_2020$date)

date <- as.Date("2019-08-23") + seq(0, along.with = c(NA, HPIV_2020$confirm))
...
write_rds(fm2_2020, file = "//data//perofskyamc//HPIV_1_2_2020_biowulf_epidemia_model.rds")

####################################
## extract Rt 
####################################

HPIV_rt_2020_1_2 = extract_Rt(fm2_2020)

all_HPIV_rt_1_2 = HPIV_rt_2020_1_2 %>% group_by(group,tag,level) %>% tidyr::complete(date=full_seq(date,1))%>% ungroup()
all_HPIV_rt_1_2$organism = "HPIV-1 + HPIV-2"
write_rds(all_HPIV_rt_1_2, file = "//data//perofskyamc//HPIV_1_2_biowulf_epidemia_model_rt_only.rds") #rt only
