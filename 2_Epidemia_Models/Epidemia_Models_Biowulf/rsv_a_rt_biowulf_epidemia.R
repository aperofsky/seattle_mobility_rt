...
## RSV A 2019
####################################
RSV_A_2019 <- filter_fun(pathogen_df %>% filter(date < as.Date("2019-06-30") & date > as.Date("2018-11-01"))) %>% filter(date > as.Date("2018-11-25"))
# range(RSV_A_2019$date)
# ggplot(RSV_A_2019)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2018-11-25") + seq(0, along.with = c(NA, RSV_A_2019$confirm))

data <- data.frame(
  city = "Seattle", cases = c(NA, RSV_A_2019$confirm),
  date = date
)
...
## RSV A 2020
####################################
RSV_A_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-09-25") & date < as.Date("2020-04-30"))) %>% filter(date > as.Date("2019-10-12"))
# range(RSV_A_2020$date)
# ggplot(RSV_A_2020)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2019-10-12") + seq(0, along.with = c(NA, RSV_A_2020$confirm))

data <- data.frame(
  city = "Seattle", cases = c(NA, RSV_A_2020$confirm),
  date = date
)
...
####################################
all_rsv_a_rt <- bind_rows(RSV_A_2019, RSV_A_2020)
all_rsv_a_rt <- all_rsv_a_rt %>%
  group_by(group, tag, level) %>%
  tidyr::complete(date = full_seq(date, 1)) %>%
  ungroup()
all_rsv_a_rt$organism <- "RSV A"
# range(all_rsv_a_rt$date)
write_rds(all_rsv_a_rt, file = "//data//perofskyamc//RSV_A_biowulf_epidemia_model_rt_only.rds") # rt only
