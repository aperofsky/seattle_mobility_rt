...
H1N1_2019 <- filter_fun(pathogen_df %>%
  filter(date < as.Date("2019-06-01") & date > as.Date("2018-11-14"))) %>%
  filter(date > as.Date("2018-11-28") & date < as.Date("2019-05-26"))
# ggplot(H1N1_2019)+
#   geom_line(aes(x=date,y=confirm))
# range(H1N1_2019$date)

date <- as.Date("2018-11-28") + seq(0, along.with = c(NA, H1N1_2019$confirm))
...
## H1N1 2020
####################################
H1N1_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-09-13") & date < as.Date("2020-05-15"))) %>% filter(date > as.Date("2019-09-25"))
# ggplot(H1N1_2020)+
#   geom_line(aes(x=date,y=confirm))
# range(H1N1_2020$date)

date <- as.Date("2019-09-25") + seq(0, along.with = c(NA, H1N1_2020$confirm))
