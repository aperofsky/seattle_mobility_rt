...
## RSV B 2019
####################################
RSV_B_2019 <- filter_fun(pathogen_df %>% filter(date < as.Date("2019-06-30"))) %>% filter(date > as.Date("2018-11-23"))
# range(RSV_B_2019$date)
# ggplot(RSV_B_2019)+
#   geom_line(aes(x=date,y=confirm))+
#   geom_vline(xintercept = as.Date("2018-11-28"),lty="dashed")

date <- as.Date("2018-11-23") + seq(0, along.with = c(NA, RSV_B_2019$confirm))

data <- data.frame(
  city = "Seattle", cases = c(NA, RSV_B_2019$confirm),
  date = date
)
...
RSV_B_si <- R0::generation.time(type = "gamma", c(7.5, 2.5), truncate = 26)

# basic model
inf <- epiinf(gen = RSV_B_si$GT)

# extended model
# treat infections as latent parameters
inf_ext <- epiinf(
  gen = RSV_B_si$GT, latent = TRUE,
...
write_rds(fm2, file = "//data//perofskyamc//RSV_B_2019_biowulf_epidemia_model.rds")

# fm2 = read_rds("Epidemia_Models/epidemia_rt_output/RSV_B_2019_biowulf_epidemia_model.rds")

RSV_B_2019 <- extract_Rt(fm2)

#####################################
## RSV B 2020
####################################
RSV_B_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-09-30") & date < as.Date("2020-05-01"))) %>% filter(date > as.Date("2019-10-24"))
# ggplot(RSV_B_2020)+
#   geom_line(aes(x=date,y=confirm))
# range(RSV_B_2020$date)

date <- as.Date("2019-10-24") + seq(0, along.with = c(NA, RSV_B_2020$confirm))

data <- data.frame(
  city = "Seattle", cases = c(NA, RSV_B_2020$confirm),
  date = date
)
...
write_rds(fm2_2020, file = "//data//perofskyamc//RSV_B_2020_biowulf_epidemia_model.rds")

RSV_B_2020 <- extract_Rt(fm2_2020)

#####################################
## RSV B Rt 2021
####################################
RSV_B_2021 <- filter_fun(pathogen_df %>% filter(date > as.Date("2021-05-01") & date < as.Date("2022-07-15"))) %>% filter(date > as.Date("2021-05-16") & date <= as.Date("2022-02-20"))
# ggplot(RSV_B_2021)+
#   geom_line(aes(x=date,y=confirm))+
#   geom_vline(xintercept = as.Date("2021-06-21"))
# range(RSV_B_2021$date)

date <- as.Date("2021-05-16") + seq(0, along.with = c(NA, RSV_B_2021$confirm))

data <- data.frame(
  city = "Seattle", cases = c(NA, RSV_B_2021$confirm),
  date = date
)
...
write_rds(fm2_2021, file = "//data//perofskyamc//RSV_B_2021_biowulf_epidemia_model.rds")

RSV_B_2021 <- extract_Rt(fm2_2021)

####################################
## combine all Rt output
####################################
all_rsv_b_rt <- bind_rows(RSV_B_2019, RSV_B_2020, RSV_B_2021)

all_rsv_b_rt <- all_rsv_b_rt %>%
  group_by(group, tag, level) %>%
  tidyr::complete(date = full_seq(date, 1)) %>%
  ungroup()
all_rsv_b_rt$organism <- "RSV B"

write_rds(all_rsv_b_rt, file = "//data//perofskyamc//RSV_B_biowulf_epidemia_model_rt_only.rds") # rt only
