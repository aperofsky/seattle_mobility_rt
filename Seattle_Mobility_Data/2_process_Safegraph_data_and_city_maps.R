...
# Sys.setenv(CENSUS_KEY = "your key here") #for tigris package
...
tract_puma <- read.table("Seattle_Mobility_Data/mobility_data/tract_puma_mapping.txt", sep = ",", header = T)[, c(2:3)]
...
# Metadata for CBGs in WA state
block_group_metdata <- block_groups("WA", year = 2019)
head(block_group_metdata)

# CBG numbers for King, Pierce, Snohomish
...
df <- home_and_visitor_panel %>%
  as_tibble() %>%
  group_by(start_date) %>%
  summarize(total = sum(home_panel_poi_cbg))
head(df)
...
## check for correction to home panel size
# df <- home_and_visitor_panel4 %>%
#   group_by(start_date) %>%
#   summarize(total = sum(home_panel_poi_state, na.rm = T))
#
# ggplot(df) +
...
write_rds(home_and_visitor_panel4, file = paste0(dir, "home_and_visitor_panel_2018_2022.rds"))
# home_and_visitor_panel4 <- read_rds(paste0(dir, "home_and_visitor_panel_2018_2022.rds"))
...
####################################################
weekly_visitors_cbg_all_years <- read_rds(paste0(dir, "SG_patterns_weekly_visitor_home_cbg_all_years_2018_to_2022.rds"))
...
write_rds(seattle_weekly_visitors_cbg_all_years, file = paste0(dir, "seattle_weekly_visitors_cbg_all_years_2018_2022.rds"))
...
####################################################
# source: https://data-seattlecitygis.opendata.arcgis.com/datasets/SeattleCityGIS::census-block-groups-2010/about
## Seattle CBGs
cbg_gps <- read.csv("Seattle_Mobility_Data/mobility_data/Seattle_Census_Block_Groups_2010.csv", colClasses = c("GEOID10" = "character"))
head(cbg_gps)
...
write_rds(weekly_visitors_cbg_within_seattle, file = paste0(dir, "weekly_visitors_cbg_within_seattle_2018_2022.rds"))
# weekly_visitors_cbg_within_seattle <- read_rds(paste0(dir, "weekly_visitors_cbg_within_seattle_2018_2022.rds"))
...
mygraph <- graph.data.frame(network_df, directed = F)
...
feb_2019_map_w_deg_log <- plot_grid(feb_2019_map, feb_deg_log, nrow = 2, rel_heights = c(6, 2))
...
#   filename = "SG_figures/within_seattle_movement_up_to_Jan_2022.png",
#   base_width = 14, base_height = 5
# )
...
## Figure 3
save_plot(map_combined2,
  filename = "Seattle_Mobility_Data/mobility_figures/fig_3_within_seattle_movement_up_to_Jan_2022_with_log_trans_deg_histogram.png",
  base_width = 16, base_height = 8
)
...
# county_visitors_out_of_county_within_state %>%
#   group_by(visitor_county) %>%
#   summarize(mean = mean(num_visits, na.rm = T)) %>%
#   arrange(-mean) %>%
#   filter(mean < 1000) %>%
...
## get rid of counties with low numbers of visits (more volatile time serie)
...
write_rds(all_between_movement, file = paste0(dir, "within_and_between_cbg_movement_indicators_up_to_2022_09_26.rds"))
# all_between_movement <- read_rds(paste0(dir, "within_and_between_cbg_movement_indicators_up_to_2022_09_26.rds"))
...
daily_visits_naics_all_years <- read_rds(paste0(dir, "SG_patterns_naics_visits_by_day_2018_to_2022.rds"))
...
write_rds(seattle_daily_visits_naics_all_years, file = paste0(dir, "seattle_daily_visits_naics.rds"))
# seattle_daily_visits_naics_all_years = read_rds("SG_data/seattle_daily_visits_naics.rds")
