########################################################################################
## extract Rt estimates from epidemia object
########################################################################################
...
########################################################################################
## Loess smoothing from Huisman et al. eLife https://doi.org/10.7554/eLife.71345
########################################################################################
...
########################################################################################
# subset of model candidates that do not include collinear variables (above a certain threshold)
########################################################################################

## from dredge() subset demo in MuMIn package
...
## subset for gams
subset_cor <- function(df = hmpv_df_lim_scaled, pathogen = "hmpv") {
  red <- data.frame(df[!(names(df) %in% c("date", "week", pathogen))])
  cov = ncol(red)
  smat <- outer(1:cov, 1:cov, vCorrelated, data = red)
  nm <- colnames(red)
  nm <- paste0("s(", nm, ")")
  dimnames(smat) <- list(nm, nm)
  smat["s(week_index)",]<- c(rep(TRUE,ncol(smat)-1),NA) #always keep temporal trend
  return(smat)
}
