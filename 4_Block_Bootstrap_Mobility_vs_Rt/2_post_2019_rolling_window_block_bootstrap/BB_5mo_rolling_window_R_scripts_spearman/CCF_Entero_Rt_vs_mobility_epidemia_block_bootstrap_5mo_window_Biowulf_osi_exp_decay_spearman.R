...
comb_weekly <- combined %>%
  ...
  group_by(epi_date) %>%
  summarize_at(c("entero", metrics), ~ mean(.x, na.rm = T))

actual_data_and_perm <- ts_block_bootstrap_sp(df1=comb_weekly,pathogen="entero",window=10,l=0.077,perms=1000)

save(actual_data_and_perm, file = "//data//perofskyamc//entero_mobility_CCF_5mo_sliding_window_actual_and_null_output_osi_spearman.RData")
