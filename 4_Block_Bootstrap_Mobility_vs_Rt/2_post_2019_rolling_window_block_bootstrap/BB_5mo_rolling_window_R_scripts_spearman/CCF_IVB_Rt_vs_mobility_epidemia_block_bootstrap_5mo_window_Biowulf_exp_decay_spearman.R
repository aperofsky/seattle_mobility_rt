...
combined["ivb"][is.na(combined["ivb"])] <- 0
...
comb_weekly <- combined %>%
  ...
  group_by(epi_date) %>%
  summarize_at(c("ivb", metrics), ~ mean(.x, na.rm = T))
comb_weekly["ivb"][is.na(comb_weekly["ivb"])] <- 0

actual_data_and_perm <- ts_block_bootstrap_sp(df1=comb_weekly,pathogen="ivb",window=10,l=0.077,perms=1000)

save(actual_data_and_perm, file = "//data//perofskyamc//ivb_mobility_CCF_5mo_sliding_window_actual_and_null_output_spearman.RData")
