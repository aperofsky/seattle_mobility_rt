...
rt_df_wide = rt_df %>% dplyr::select(date,median,organism)%>% distinct() %>%
  pivot_wider(names_from="organism",values_from="median")%>%
  dplyr::filter(date>as.Date("2018-11-20"))

names(rt_df_wide)[2:14]<- c("rsv_a","rsv_b",
                            "h1n1","h3n2","ivb","rhino",
...
metrics= c("within_neighborhood_movement",
...
comb_weekly = combined %>%
  ...
  filter(epi_date>=as.Date("2019-12-01") & epi_date< as.Date("2022-05-22"))%>%
  group_by(epi_date) %>%
...
y<- function(X,Y,l=0.07){
  df = df1[,c("epi_date",as.character(X),"covid")] %>% 
  ...
  exp_plot = ggplot()+ geom_line(aes(x=seq(1:length(exp_w)),y=exp_w))
  exp_plot
  ...
  actual_data <- data.frame(mobility_metric=as.character(X),
...
y<- function(X=metrics,Y=weeks,perms=1000,l=0.07){
  actual_hosp = df1[,c("epi_date","covid")] %>% 
    filter(epi_date>=lubridate::as_date(Y)-(7*window) & epi_date<=lubridate::as_date(Y)+(7*window)) %>% as.data.frame()
...
system.time(result_list <- mcmapply(FUN=y,X=tmp[1,],Y=tmp[2,],l=0.07,perms=1000,SIMPLIFY = F,mc.cores=mc.cores))
# analysis takes 2.5 hours
#     user    system   elapsed 
# 57208.736   138.781  9241.081 
...
d2 <- null_and_obs_ccf %>%
  group_by(mobility_metric, start_week) %>%
  summarize(lower = quantile(abs(max_ccf), probs = .05),
...
save(actual_data_and_perm ,file = "//data//perofskyamc//covid_mobility_CCF_5mo_sliding_window_actual_and_null_output.RData")