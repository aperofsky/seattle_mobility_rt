...
comb_weekly <- combined %>%
  ...
  filter(epi_date >= as.Date("2019-01-01") & epi_date < as.Date("2022-05-22")) %>%
  group_by(epi_date) %>%
...
save(actual_data_and_perm, file = "//data//perofskyamc//adeno_mobility_CCF_5mo_sliding_window_actual_and_null_output.RData")
