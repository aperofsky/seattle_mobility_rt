...
# combined$inv_oxford_stringency_index = 100- combined$oxford_stringency_index
...
combined["rsv_b"][is.na(combined["rsv_b"])] <- 0

comb_weekly <- combined %>%
  mutate_at(vars(all_of(metrics)), ~ scale(.x) %>% as.vector()) %>%
  filter(epi_date >= as.Date("2021-01-01") & epi_date < as.Date("2022-05-01")) %>%
  group_by(epi_date) %>%
  summarize_at(c("rsv_b", metrics), ~ mean(.x, na.rm = T))
comb_weekly["rsv_b"][is.na(comb_weekly["rsv_b"])] <- 0
...
y <- function(X, Y, l = 0.07) {
  df <- df1[, c("epi_date", as.character(X), "rsv_b")] %>%
...
y <- function(X = metrics, Y = weeks, perms = 1000, l = 0.07) {
  actual_hosp <- df1[, c("epi_date", "rsv_b")] %>%
    filter(epi_date >= lubridate::as_date(Y) - (7 * window) & epi_date <= lubridate::as_date(Y) + (7 * window)) %>%
    as.data.frame()
...
system.time(result_list <- mcmapply(FUN = y, X = tmp[1, ], Y = tmp[2, ], l = 0.07, perms = 1000, SIMPLIFY = F, mc.cores = mc.cores)) # analysis takes 2.5 hours
...
save(actual_data_and_perm, file = "//data//perofskyamc//rsv_b_mobility_CCF_5mo_sliding_window_actual_and_null_output_rebound_2021.RData")
