...
y <- function(X, Y, l = 0.07) {
  df <- df1[, c("epi_date", as.character(X), "ivb")] %>%
...
y <- function(X = metrics, Y = weeks, perms = 1000, l = 0.07) {
  actual_hosp <- df1[, c("epi_date", "ivb")] %>%
    filter(epi_date >= lubridate::as_date(Y) - (7 * window) & epi_date <= lubridate::as_date(Y) + (7 * window)) %>%
    as.data.frame()
...
save(actual_data_and_perm, file = "//data//perofskyamc//ivb_mobility_CCF_5mo_sliding_window_actual_and_null_output.RData")
