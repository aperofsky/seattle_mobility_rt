...
combined["rsv_a"][is.na(combined["rsv_a"])] <- 0
...
comb_weekly <- combined %>%
  ...
  group_by(epi_date) %>%
  summarize_at(c("rsv_a", metrics), ~ mean(.x, na.rm = T))
comb_weekly["rsv_a"][is.na(comb_weekly["rsv_a"])] <- 0
...
y <- function(X, Y, l = 0.07) {
  df <- df1[, c("epi_date", as.character(X), "rsv_a")] %>%
...
y <- function(X = metrics, Y = weeks, perms = 1000, l = 0.07) {
  actual_hosp <- df1[, c("epi_date", "rsv_a")] %>%
    filter(epi_date >= lubridate::as_date(Y) - (7 * window) & epi_date <= lubridate::as_date(Y) + (7 * window)) %>%
    as.data.frame()
...
save(actual_data_and_perm, file = "//data//perofskyamc//rsv_a_mobility_CCF_5mo_sliding_window_actual_and_null_output.RData")
