...
# combined[!is.na(combined$oxford_stringency_index),]
comb_weekly <- combined %>%
  mutate_at(vars(all_of(metrics)), ~ scale(.x) %>% as.vector()) %>%
  filter(epi_date >= as.Date("2020-02-01") & epi_date < as.Date("2022-05-22")) %>%
  group_by(epi_date) %>%
  summarize_at(c("adeno", metrics), ~ mean(.x, na.rm = T))
...
y <- function(X, Y, l = 0.07) {
  df <- df1[, c("epi_date", as.character(X), "adeno")] %>%
...
y <- function(X = metrics, Y = weeks, perms = 1000, l = 0.07) {
  actual_hosp <- df1[, c("epi_date", "adeno")] %>%
    filter(epi_date >= lubridate::as_date(Y) - (7 * window) & epi_date <= lubridate::as_date(Y) + (7 * window)) %>%
    as.data.frame()
...
# analysis takes 1.5 hours
#     user    system   elapsed
# 41226.369   197.978  5428.451
...
save(actual_data_and_perm, file = "//data//perofskyamc//adeno_mobility_CCF_5mo_sliding_window_actual_and_null_output_osi.RData")
