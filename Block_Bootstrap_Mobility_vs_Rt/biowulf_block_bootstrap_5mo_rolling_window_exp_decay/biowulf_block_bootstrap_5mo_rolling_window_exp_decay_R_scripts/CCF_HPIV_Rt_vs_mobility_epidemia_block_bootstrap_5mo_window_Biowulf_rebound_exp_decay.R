...
combined["hpiv_3_4"][is.na(combined["hpiv_3_4"])] <- 0
...
comb_weekly <- combined %>%
  ...
  group_by(epi_date) %>%
  summarize_at(c("hpiv_3_4", metrics), ~ mean(.x, na.rm = T))
comb_weekly["hpiv_3_4"][is.na(comb_weekly["hpiv_3_4"])] <- 0
...
y <- function(X, Y, l = 0.07) {
  df <- df1[, c("epi_date", as.character(X), "hpiv_3_4")] %>%
...
y <- function(X = metrics, Y = weeks, perms = 1000, l = 0.07) {
  actual_hosp <- df1[, c("epi_date", "hpiv_3_4")] %>%
    filter(epi_date >= lubridate::as_date(Y) - (7 * window) & epi_date <= lubridate::as_date(Y) + (7 * window)) %>%
    as.data.frame()
...
save(actual_data_and_perm, file = "//data//perofskyamc//hpiv_3_4_mobility_CCF_5mo_sliding_window_actual_and_null_output_rebound_2021.RData")
