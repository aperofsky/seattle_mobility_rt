...
combined["hpiv_1_2"][is.na(combined["hpiv_1_2"])] <- 0

comb_weekly <- combined %>%
  mutate_at(vars(all_of(metrics)), ~ scale(.x) %>% as.vector()) %>%
  filter(epi_date >= as.Date("2019-06-01") & epi_date < as.Date("2020-04-26")) %>%
  group_by(epi_date) %>%
  summarize_at(c("hpiv_1_2", metrics), ~ mean(.x, na.rm = T))
comb_weekly["hpiv_1_2"][is.na(comb_weekly["hpiv_1_2"])] <- 0
...
y <- function(X, Y, l = 0.07) {
  # print(X)
  # print(Y)
  df <- df1[, c("epi_date", as.character(X), "hpiv_1_2")] %>%
...
y <- function(X = metrics, Y = weeks, perms = 1000, l = 0.07) {
  actual_hosp <- df1[, c("epi_date", "hpiv_1_2")] %>%
    filter(epi_date >= lubridate::as_date(Y) - (7 * window) & epi_date <= lubridate::as_date(Y) + (7 * window)) %>%
    as.data.frame()
...
save(actual_data_and_perm, file = "//data//perofskyamc//hpiv_1_2_mobility_CCF_5mo_sliding_window_actual_and_null_output.RData")
