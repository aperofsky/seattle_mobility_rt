...
var_target <- "rhino" # dependent variable
var_regressors_GT <- select(rhino_df_scaled,-date,-rhino,-covid) %>% names()
var_regressors_GT_COVID <- select(rhino_df_scaled,-date,-rhino) %>% names()
...
min_date = min(rhino_df_scaled$date)
...
head(reference_dates[reference_dates[["num_days"]] >= -3, ])
head(reference_dates[reference_dates[["num_days"]] == 45, ])

rdf = left_join(rhino_df_scaled,reference_dates)
...
rhino_agg = rdf %>% 
  dplyr::select(date,num_days,epi_year,epi_week,rhino)%>%
  mutate_at(vars(rhino), .funs=lag_functions)

rhino_agg <- gather(rhino_agg, key = "metric", value = "rhino",rhino:AR14)
head(rhino_agg[rhino_agg$metric == "AR01", ])
rhino_agg[complete.cases(rhino_agg),]

temp_rhino <- rhino_agg %>% 
  rename(val = rhino) %>%
  filter(num_days >=15 & num_days <= day_max)

sgtrends_rhino = rhino_df_scaled%>% 
  ...
  pivot_longer(cols = all_of(unique(c(var_regressors_GT,var_regressors_GT_COVID))),
               # rsv_a:ivb,hpiv:covid),
               names_to = "metric",values_to = "val")%>%
  filter(num_days >=15 & num_days <= day_max)
unique(sgtrends_rhino$metric)

sg = unique(sgtrends_rhino$metric)
sg

col_order = c("date","num_days","epi_year","epi_week", "rhino", sprintf("AR%02d", 1:14), sg)
glm_rhino <- rbind(temp_rhino, sgtrends_rhino)
glm_rhino <- spread(glm_rhino, key = "metric", value = "val")
glm_rhino <- glm_rhino[col_order]
head(glm_rhino)
tail(glm_rhino)
...
for (t_predict_start in t0_predict_start:tmax_predict_start) {
  ...
  glm_train <- filter(glm_rhino, num_days >= t_train_start & num_days <= t_train_end)
  glm_predict <- filter(glm_rhino, num_days >= t_predict_start & num_days <= t_predict_end)
  ...
  y_train <- glm_train[, var_target] %>% pull(rhino)
  ...
  data_epi <- glm_predict[, "rhino"] %>% t %>% data.frame 
...
########################################################################################
## Figure S20: Overlay model predictions on observed rhino Rt and incidence
########################################################################################
...
ci_df = rt_df %>% filter(organism=="RV") %>%
...
rhino_results = full_df %>% filter(organism =="Rhinovirus")%>%droplevels()

rhino_results = rhino_results %>%
...
r = ggplot() +
  ...
  geom_ribbon(data=rhino_results %>% 
                filter(date>=as.Date("2019-01-01")&date<=as.Date("2022-05-01")),
  ...
  geom_line(data = ci_df %>% 
              filter(date>=as.Date("2019-01-01")&date<=as.Date("2022-05-01")),
            aes(x = date, y=median,colour = "Rhino Rt"),linewidth=1.2) +
  ...
  scale_color_manual(name=NULL,
                     ...
                     breaks=c("Rhino Rt","AR-SG-COVID","SG-COVID","SG","AR"),
                     labels=c("hRV Rt","AR-Mobility-COVID","Mobility-COVID","Mobility","AR"))+
  background_grid()+
...
rmse(predicted=res_GT_COVID$predict_7d_ahead,actual=res_ARGO2$data_7d_ahead)#0.058
rmse(predicted=res_GT$predict_7d_ahead, actual=res_ARGO2$data_7d_ahead)#0.058
rmse(predicted=res_AR$predict_7d_ahead, actual=res_ARGO2$data_7d_ahead)# 0.007

mae(predicted=res_ARGO$predict_7d_ahead, actual=res_ARGO2$data_7d_ahead)# 0.008
mae(predicted=res_GT_COVID$predict_7d_ahead,actual=res_ARGO2$data_7d_ahead)#0.042
mae(predicted=res_GT$predict_7d_ahead, actual=res_ARGO2$data_7d_ahead)#0.041
mae(predicted=res_AR$predict_7d_ahead, actual=res_ARGO2$data_7d_ahead)# 0.0051

MAPE(res_ARGO$predict_7d_ahead, res_ARGO2$data_7d_ahead)#0.008
MAPE(res_GT_COVID$predict_7d_ahead, res_ARGO2$data_7d_ahead)#0.043
MAPE(res_GT$predict_7d_ahead, res_ARGO2$data_7d_ahead)#0.042
MAPE(res_AR$predict_7d_ahead, res_ARGO2$data_7d_ahead)#0.005

cor(res_ARGO$predict_7d_ahead, res_ARGO2$data_7d_ahead,  method = "pearson", use = "complete.obs")#1.0
cor(res_GT_COVID$predict_7d_ahead, res_ARGO2$data_7d_ahead,  method = "pearson", use = "complete.obs")#0.93
cor(res_GT$predict_7d_ahead, res_ARGO2$data_7d_ahead,  method = "pearson", use = "complete.obs")#0.93
...
save_plot(com, filename = "figures/fig_s19_rhinovirus_rt_predictive_model_fit_7d_ahead.png",base_width = 12,base_height = 10)
...
rmse(res_ARGO_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)# 0.006
rmse(res_GT_COVID_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.054
rmse(res_GT_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.048
rmse(res_AR_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.005

mae(res_ARGO_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)# 0.005
mae(res_GT_COVID_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.042
mae(res_GT_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.036
mae(res_AR_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.004

MAPE(res_ARGO_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.007
MAPE(res_GT_COVID_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.057
MAPE(res_GT_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.05
MAPE(res_AR_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead)#0.006

cor(res_ARGO_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead,  method = "pearson", use = "complete.obs")#1.0
cor(res_GT_COVID_sah$predict_7d_ahead, res_ARGO2_sah$data_7d_ahead,  method = "pearson", use = "complete.obs")#0.93
