...
### RSV B
############################
rsv_b_df_lim <- com_2021_df_lm_scaled %>%
  dplyr::select(date, time, week, all_of(cols2), rsv_b) %>%
  filter(rsv_b > 0)

week_df <- data.frame(
  week = unique(rsv_b_df_lim$week),
  week_index = seq(1:length(unique(rsv_b_df_lim$week)))
)
week_df

rsv_b_df_lim <- left_join(rsv_b_df_lim, week_df, by = "week")

fm <- paste("s(", names(rsv_b_df_lim[!(names(rsv_b_df_lim) %in% c("date", "time", "week", "rsv_b", "child_day_care"))]), ")", sep = "", collapse = " + ")
...
hmpv_df_lim <- left_join(hmpv_df_lim, week_df, by = "week")
range(hmpv_df_lim$date)
...
############################
scov_229E_OC43_df_lim <- com_2021_df_lm %>%
...
p1 <- draw(scov_229E_OC43_b1, residuals = T, select = "s(out_of_state_movement)") & theme_bw() & ggtitle("s(out-of-state inflow)") & xlab("out-of-state inflow")
p1

p2 <- draw(scov_229E_OC43_b1, residuals = T, select = "s(week_index)") & theme_bw() & ggtitle("s(week)") & xlab("week")
p2

scov_229E_OC43_p_all <- p1 + p2 + plot_layout(ncol = 2) +
...
p1 <- draw(scov_HKU1_NL63_b1, residuals = T, select = "s(within_city_movement)") & theme_bw() & ggtitle("s(between-CBG movement)") & xlab("between-CBG movement")
p1
...
fm <- paste("s(", names(hpiv_3_4_df_lim[!(names(hpiv_3_4_df_lim) %in% c("date", "time", "week", "hpiv_3_4", "child_day_care"))]), ")", sep = "", collapse = " + ")
...
### rhino
############################
rhino_df_lim <- com_2021_df_lm %>%
  dplyr::select(date, time, week, all_of(cols2), rhino) %>%
  filter(rhino > 0)
range(rhino_df_lim$date)

week_df <- data.frame(
  week = unique(rhino_df_lim$week),
  week_index = seq(1:length(unique(rhino_df_lim$week)))
)
week_df

rhino_df_lim <- left_join(rhino_df_lim, week_df, by = "week")
range(rhino_df_lim$date)

fm <- paste("s(", names(rhino_df_lim[!(names(rhino_df_lim) %in% c("date", "time", "week", "rhino", "child_day_care"))]), ")", sep = "", collapse = " + ")
fm <- as.formula(paste("rhino ~", fm))
fm
b1 <- mgcv::gam(
  formula = fm, data = rhino_df_lim, method = "ML",
  gamma = 2, select = TRUE, na.action = "na.fail", family = Gamma(link = "log")
)
...
### adenovirus
############################
adeno_df_lim <- com_2021_df_lm %>%
  dplyr::select(date, time, week, all_of(cols2), adeno) %>%
  filter(adeno > 0)
range(adeno_df_lim$date)

week_df <- data.frame(
  week = unique(adeno_df_lim$week),
  week_index = seq(1:length(unique(adeno_df_lim$week)))
)
week_df

adeno_df_lim <- left_join(adeno_df_lim, week_df, by = "week")

fm <- paste("s(", names(adeno_df_lim[!(names(adeno_df_lim) %in% c("date", "time", "week", "adeno", "child_day_care"))]), ")", sep = "", collapse = " + ")
fm <- as.formula(paste("adeno ~", fm))
fm
b1 <- mgcv::gam(
  formula = fm, data = adeno_df_lim, method = "ML",
  gamma = 2, select = TRUE, na.action = "na.fail", family = Gamma(link = "log")
)
dd <- dredge(b1, m.lim = c(1, 3), fixed = "s(week_index)")
subset(dd, delta < 4)
...
########################################################################################
all_path_2021 <- plot_grid(rsv_b_p_all,
  scov_229E_OC43_p_all, scov_HKU1_NL63_p_all,
  hmpv_p_all,
  hpiv_3_4_p_all,
...
save_plot(all_path_2021, base_height = 10, base_width = 20, filename = "figures/fig_s17_partial_effects_endemic_pathogens_omicron_2021_2022.png")
