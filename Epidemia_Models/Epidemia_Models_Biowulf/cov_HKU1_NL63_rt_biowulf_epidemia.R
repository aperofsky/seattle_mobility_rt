...
####################################
seasonal_CoV_2019 <- filter_fun(pathogen_df %>% filter(date < as.Date("2019-07-30") & date> as.Date("2018-11-13")))
...
####################################
seasonal_CoV_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-10-09") & date < as.Date("2020-05-10")))

# ggplot(seasonal_CoV_2020)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2019-10-09") + seq(0, along.with = c(NA, seasonal_CoV_2020$confirm))
...
seasonal_CoV_2021 <- filter_fun(pathogen_df %>% filter(date > as.Date("2021-01-14") & date < as.Date("2022-10-20"))) %>% filter(date < as.Date("2022-07-01"))

# ggplot(seasonal_CoV_2021)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2021-01-14") + seq(0, along.with = c(NA, seasonal_CoV_2021$confirm))
...
####################################
all_seasonal_CoV_HKU1_NL63_rt = bind_rows(seasonal_CoV_HKU1_NL63_rt_2019,
                                          seasonal_CoV_HKU1_NL63_rt_2020,
                                          seasonal_CoV_HKU1_NL63_rt_2021)

all_seasonal_CoV_HKU1_NL63_rt = all_seasonal_CoV_HKU1_NL63_rt %>% group_by(group,tag,level) %>% tidyr::complete(date=full_seq(date,1))%>% ungroup()
all_seasonal_CoV_HKU1_NL63_rt$organism = "Seasonal CoV HKU1 + NL63"

write_rds(all_seasonal_CoV_HKU1_NL63_rt, file = "//data//perofskyamc//seasonal_CoV_HKU1_NL63_epidemia_model_rt_only.rds") #rt only
