...
hMPV_2019 <- filter_fun(pathogen_df %>% filter(date > as.Date("2018-11-14") & date < as.Date("2019-08-30")))

# ggplot(hMPV_2019)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2018-11-14") + seq(0, along.with = c(NA, hMPV_2019$confirm))
...
## hMPV 2020
####################################
hMPV_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-09-13") & date < as.Date("2020-05-29")))

# ggplot(hMPV_2020)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2019-09-13") + seq(0, along.with = c(NA, hMPV_2020$confirm))
...
## hMPV 2021
####################################
hMPV_2021 <- filter_fun(pathogen_df %>% filter(date > as.Date("2021-03-30") & date < as.Date("2022-10-15"))) %>% filter(date < as.Date("2022-07-01"))

# ggplot(hMPV_2021)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2021-03-30") + seq(0, along.with = c(NA, hMPV_2021$confirm))
...
all_hMPV_rt = all_hMPV_rt %>% group_by(group,tag,level) %>% tidyr::complete(date=full_seq(date,1))%>% ungroup()
