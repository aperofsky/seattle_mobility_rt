...
####################################
seasonal_CoV_2019 <- filter_fun(pathogen_df %>% filter(date>as.Date("2018-11-13") & date < as.Date("2019-08-15"))) %>% filter(date < as.Date("2019-07-30"))

date <- as.Date("2018-11-13") + seq(0, along.with = c(NA, seasonal_CoV_2019$confirm))
...
####################################
seasonal_CoV_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-10-30") & date < as.Date("2020-05-24")))

date <- as.Date("2019-10-30") + seq(0, along.with = c(NA, seasonal_CoV_2020$confirm))
...
####################################
seasonal_CoV_2021 <- filter_fun(pathogen_df %>% filter(date > as.Date("2021-01-30") & date < as.Date("2022-10-05"))) %>% filter(date < as.Date("2022-07-01"))

date <- as.Date("2021-01-30") + seq(0, along.with = c(NA, seasonal_CoV_2021$confirm))
...
all_seasonal_CoV_229E_OC43_rt = bind_rows(seasonal_CoV_229E_OC43_rt_2019,
                                          seasonal_CoV_229E_OC43_rt_2020,
                                          seasonal_CoV_229E_OC43_rt_2021)

all_seasonal_CoV_229E_OC43_rt = all_seasonal_CoV_229E_OC43_rt %>% group_by(group,tag,level) %>% tidyr::complete(date=full_seq(date,1))%>% ungroup()
all_seasonal_CoV_229E_OC43_rt$organism = "Seasonal CoV 229E + OC43"

write_rds(all_seasonal_CoV_229E_OC43_rt, file = "//data//perofskyamc//seasonal_CoV_229E_OC43_epidemia_model_rt_only.rds") #rt only