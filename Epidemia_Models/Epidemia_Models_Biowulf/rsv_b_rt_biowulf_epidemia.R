...
RSV_B_2019 <- filter_fun(pathogen_df %>% filter(date < as.Date("2019-06-30") & date>as.Date("2018-11-14")))

date <- as.Date("2018-11-14") + seq(0, along.with = c(NA, RSV_B_2019$confirm))
...
## RSV B 2020
####################################
RSV_B_2020 <- filter_fun(pathogen_df %>% filter(date > as.Date("2019-09-30") & date < as.Date("2020-05-15")))

# ggplot(RSV_B_2020)+
#   geom_line(aes(x=date,y=confirm))

date <- as.Date("2019-09-30") + seq(0, along.with = c(NA, RSV_B_2020$confirm))
...
RSV_B_2021 <- filter_fun(pathogen_df %>% filter(date > as.Date("2021-05-01") & date < as.Date("2022-07-15"))) %>%
  filter(date < as.Date("2022-07-01"))

date <- as.Date("2021-05-01") + seq(0, along.with = c(NA, RSV_B_2021$confirm))
...
all_rsv_b_rt = all_rsv_b_rt %>% group_by(group,tag,level) %>% tidyr::complete(date=full_seq(date,1))%>% ungroup()
all_rsv_b_rt$organism = "RSV B"

write_rds(all_rsv_b_rt, file = "//data//perofskyamc//RSV_B_epidemia_model_rt_only.rds") #rt only
